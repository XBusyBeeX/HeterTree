...
public:
	...
	int		  toInt()		  { return *static_cast<const int*>(data()); }
	long long toInt64()       { return *static_cast<const long long*>(data()); }
	float	  toFloat()	      { return *static_cast<const float*>(data()); }
	double	  toDouble()      { return *static_cast<const double*>(data()); }
	std::string toString() {
		return std::string(static_cast<const char*>(data()), size_t(size()));
	}
	std::wstring toWString() {
		return std::wstring(static_cast<const wchar_t*>(data()), size_t(size()/sizeof(wchar_t)));
	}
